...
const addMoneyAccount = `-- name: AddMoneyAccount :one
UPDATE accounts
...
func (q *Queries) AddMoneyAccount(ctx context.Context, arg AddMoneyAccountParams) (Accounts, error) {
	row := q.db.QueryRowContext(ctx, addMoneyAccount, arg.Amount, arg.ID)
	var i Accounts
	err := row.Scan(
		&i.ID,
		&i.Owner,
